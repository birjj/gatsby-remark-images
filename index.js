...
module.exports = ({
  ...
  const findParentLinks = ({
    children
  }) => children.some(node => node.type === `html` && !!node.value.match(/<a /) || node.type === `link`); // Get all the available definitions in the markdown tree


  const definitions = getDefinitions(markdownAST); // This will allow the use of html image tags
  ...
  const generateImagesAndUpdateNode = async function (node, resolve, inLink, overWrites = {}) {
    ...
    const title = node.title ? node.title : alt; // Create our base image tag
    ...
    let placeholderImageData = fluidResult.base64; // if options.tracedSVG is enabled generate the traced SVG and use that as the placeholder image

    if (options.tracedSVG) {
      let args = typeof options.tracedSVG === `object` ? options.tracedSVG : {}; // Translate Potrace constants (e.g. TURNPOLICY_LEFT, COLOR_AUTO) to the values Potrace expects
    ...
    const wrapperStyle = typeof options.wrapperStyle === `function` ? options.wrapperStyle(fluidResult) : options.wrapperStyle; // Construct new image node w/ aspect ratio placeholder

    const imageCaption = options.showCaptions && getImageCaption(node, alt, defaultAlt);
    let rawHTML = `
  <span
  ...
  >
    ...
    let shouldMaxWidth = !imageCaption && wrapperStyle.indexOf("max-width:") === -1;
    rawHTML = `
    <span
      class="${imageWrapperClass}"
      style="position: relative; display: block; margin-left: auto; margin-right: auto; ${imageCaption ? `` : wrapperStyle} ${shouldMaxWidth ? `max-width: ${presentationWidth}px;` : ``}"
    >
      ${rawHTML}
    </span>
  ...
  markdownImageNodes.map(({
    ...
    const fileType = getImageInfo(node.url).ext; // Ignore gifs as we can't process them,
  ...
  rawHtmlNodes.map(({
    ...
    for (let thisImg of imageRefs) {
      ...
      const fileType = getImageInfo(formattedImgTag.url).ext; // Ignore gifs as we can't process them,
      // svgs as they are already responsive by definition

      if (isRelativeUrl(formattedImgTag.url) && fileType !== `gif` && fileType !== `svg`) {
        const rawHTML = await generateImagesAndUpdateNode(formattedImgTag, resolve, inLink);
